...
class ParseXMLTest extends PHPUnit_Framework_TestCase{
    ...
    public function testParseXML(){
        ...
        if(file_exists(dirname(__FILE__).'/output')){
            DocxTemplate::deleteDir(dirname(__FILE__).'/output');
        }
        $this->assertFalse(file_exists(dirname(__FILE__).'/output'));

        mkdir(dirname(__FILE__).'/output',0777,true);

        $document = new DOMDocument();
        $document->preserveWhiteSpace = false;
        $document->loadXML($templateDoc);

        $templatePath = dirname(__FILE__).'/output/template.docx';
        ...
        $outputPath = dirname(__FILE__).'/output/mergedOutput.docx';
